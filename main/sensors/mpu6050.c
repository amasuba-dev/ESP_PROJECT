...
esp_err_t mpu6050_init(i2c_master_bus_handle_t bus_handle)
{
    i2c_device_config_t dev_cfg = {
        ...
        .scl_speed_hz = 100000, /* 100 kHz – proven reliable on ESP32 */
    };
    ESP_ERROR_CHECK(i2c_master_bus_add_device(bus_handle, &dev_cfg, &s_dev));

    /* Verify chip identity – mirrors Arduino testConnection() */
    uint8_t who_reg = REG_WHO_AM_I;
    uint8_t who_val = 0;
    esp_err_t ret = ESP_FAIL;
    for (int attempt = 0; attempt < 5; attempt++)
    {
        ret = i2c_master_transmit_receive(s_dev, &who_reg, 1, &who_val, 1, 100);
        if (ret == ESP_OK)
            break;
        ESP_LOGW(TAG, "WHO_AM_I read attempt %d/5 failed", attempt + 1);
        vTaskDelay(pdMS_TO_TICKS(200));
    }
    if (ret != ESP_OK || who_val != MPU6050_ADDR)
    {
        ESP_LOGE(TAG, "MPU-6050 WHO_AM_I failed (got 0x%02X, expected 0x%02X) – check wiring",
                 who_val, MPU6050_ADDR);
        return ESP_FAIL;
    }
    ESP_LOGI(TAG, "WHO_AM_I OK (0x%02X)", who_val);

    /* Wake up – clear SLEEP bit in PWR_MGMT_1 */
    ret = write_reg(REG_PWR_MGMT_1, 0x00);
    if (ret != ESP_OK)
    {
        ESP_LOGE(TAG, "MPU-6050 wake failed – check wiring (SDA/SCL/VCC/GND)");
        return ret;
    }
    vTaskDelay(pdMS_TO_TICKS(100)); /* let sensor stabilise after wake */
